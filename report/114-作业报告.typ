...
当前系统*最大的不足是端到端时延偏长*：一次图片分析需串行经过 OCR 检测与 VLM 推理两个较重的阶段，且 VLM 为一次性阻塞式调用，用户要等整轮完成才能看到结果，交互体感不够顺滑。未来最主要的改进方向是*引入流式（streaming）与异步框架*——让 VLM 结果边生成边返回、OCR 与 VLM 阶段并行 / 流水化处理，并用异步任务队列避免 UI 阻塞，从而显著降低用户可感知时延。其次，识别质量依赖外部 VLM API、离线能力有限，可引入本地小模型兜底；知识图谱在卡片规模很大时的布局性能、以及 FSRS 参数个性化，也仍有进一步优化空间。
